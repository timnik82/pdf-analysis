...
SECTION_KEYWORDS = {
    "introduction": [
        "introduction", "background", "overview", "preface", "motivation",
        "background and motivation"
    ],
    "conclusion": [
        "conclusion", "conclusions", "concluding remarks", "summary",
        "summary and conclusions", "final remarks", "closing remarks",
        "general conclusions", "concluding notes"
    ],
    "future_outlook": [
        "future work", "future works", "future outlook", "future directions",
        "future research", "future perspectives", "future studies",
        "outlook", "perspectives", "outlook and perspectives",
        "perspectives and outlook", "open questions", "open challenges",
        "implications", "implications and future work", "roadmap"
    ],
    "results": [
...
CONCLUSION_INDICATORS = [
    "in conclusion", "to conclude", "in summary", "we have shown",
    "this study demonstrates", "our findings suggest", "we conclude",
    "this work has demonstrated", "the results demonstrate",
    "we have presented", "this paper has presented", "to summarize",
    "in this paper, we have", "our results show that", "we have demonstrated"
]

INTRODUCTION_INDICATORS = [
    "in this paper", "in this study", "in this work", "we present",
    "this paper presents", "the purpose of this", "the goal of this",
    "the aim of this", "this study aims", "we propose", "we introduce"
]
...
def detect_section_by_content(text, section_type="conclusion"):
    ...
    """
    indicators = CONCLUSION_INDICATORS if section_type == "conclusion" else INTRODUCTION_INDICATORS
...
def extract_sections_from_markdown(markdown_text):
    ...
    end_section_patterns = [
        re.compile(r'^(?:#+\s*)?(?:\*{0,2})References?(?:\*{0,2})\s*$', re.IGNORECASE | re.MULTILINE),
        re.compile(r'^(?:#+\s*)?(?:\*{0,2})Bibliography(?:\*{0,2})\s*$', re.IGNORECASE | re.MULTILINE),
        re.compile(r'^(?:#+\s*)?(?:\*{0,2})Acknowledg(?:e)?ments?(?:\*{0,2})\s*$', re.IGNORECASE | re.MULTILINE),
        re.compile(r'^(?:#+\s*)?(?:\*{0,2})Author\s+Contributions?(?:\*{0,2})\s*$', re.IGNORECASE | re.MULTILINE),
        re.compile(r'^(?:#+\s*)?(?:\*{0,2})Declaration\s+of\s+(?:Competing\s+)?Interests?(?:\*{0,2})\s*$', re.IGNORECASE | re.MULTILINE),
        re.compile(r'^(?:#+\s*)?(?:\*{0,2})Conflicts?\s+of\s+Interest(?:\*{0,2})\s*$', re.IGNORECASE | re.MULTILINE),
        re.compile(r'^(?:#+\s*)?(?:\*{0,2})Funding(?:\*{0,2})\s*$', re.IGNORECASE | re.MULTILINE),
        re.compile(r'^(?:#+\s*)?(?:\*{0,2})Supplementary\s+(?:Materials?|Information)(?:\*{0,2})\s*$', re.IGNORECASE | re.MULTILINE),
        re.compile(r'^(?:#+\s*)?(?:\*{0,2})Appendix(?:\*{0,2})\s*$', re.IGNORECASE | re.MULTILINE),
    ]
    ...
    for match in header_pattern.finditer(markdown_text):
        ...
        if '@' in header_content or 'mailto:' in header_content or 'http' in header_content:
            continue
    ...
    # =========================================================================
    sections_to_extract = ["introduction", "conclusion", "future_outlook", "results", "discussion"]
    ...
    if "conclusion" not in sections:
        ...
        last_portion = markdown_text[-2000:] if len(markdown_text) > 2000 else markdown_text
        if detect_section_by_content(last_portion, "conclusion"):
            ...
            if len(paragraphs) >= 3:
                ...
                sections["_conclusion_note"] = "Detected by content analysis (no explicit header found)"
...
def export_to_markdown(results, output_file):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        for i, paper in enumerate(results, 1):
            ...
            section_order = ['introduction', 'results', 'discussion', 'conclusion', 'future_outlook']
            section_titles = {
...
def process_pdfs(pdf_dir, output_file):
    ...
    for filename in files:
        ...
        try:
            ...
            section_keys = [k for k in extracted_data.keys() if not k.startswith('_') and k not in ['filename', 'doi']]
